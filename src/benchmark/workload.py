...
def generate_workload(
    ...
    """
    Generate a query workload (indices into the embedding array).
    ...
    Returns:
        Array of indices into embeddings, shape (n_queries,).
    """
    workload_type = workload_type.lower().strip()

    if workload_type == "uniform":
        return _uniform_workload(embeddings, n_queries, seed)
    elif workload_type == "clustered":
        return _clustered_workload(embeddings, n_queries, seed, n_clusters)
    elif workload_type == "bursty":
        return _bursty_workload(embeddings, n_queries, seed, n_clusters)
    else:
        raise ValueError(
...
def _uniform_workload(
    embeddings: np.ndarray, n_queries: int, seed: int
) -> np.ndarray:
    """Random uniform selection — baseline workload."""
    rng = np.random.RandomState(seed)
    N = embeddings.shape[0]
    return rng.choice(N, size=n_queries, replace=True)


def _clustered_workload(
    embeddings: np.ndarray, n_queries: int, seed: int, n_clusters: int
) -> np.ndarray:
    """
    ...
    # Cluster embeddings
    logger.info(f"Clustering {N} vectors into {actual_k} clusters...")
    ...
    # Zipf distribution over clusters (power-law)
